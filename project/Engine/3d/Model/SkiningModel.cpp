...
// 共有キャッシュの定義
std::unordered_map<std::string, std::shared_ptr<SkiningModel::GeometryCache>> SkiningModel::s_geometryCache_;

void SkiningModel::Finalize() {
	// 静的ジオメトリキャッシュをクリア
...
void SkiningModel::Initialize(const std::string& modelPath) {
	...
	if (it == s_geometryCache_.end()) {
		...
		// アニメーションデータがある → NORMAL（キーフレームアニメーション）
			...
			animationType_ = AnimationType::KEYFRAME;
			useAnimationTimer_ = true;
...
void SkiningModel::Update(float deltaTime) {
...
void SkiningModel::UpdateMatrix() {
...
void SkiningModel::Draw(Camera& useCamera, const Transform& transform) {
	...
	switch (animationType_)
	{
	case SkiningModel::AnimationType::KEYFRAME:
	{
	...
	case SkiningModel::AnimationType::SKINNING:
	{
...
void SkiningModel::DrawImGui(const char* objectName) {
	useGui_ = true;
#ifdef USE_IMGUI
...
void SkiningModel::CreateMaterialResource() {
	// MaterialResource
...
void SkiningModel::CreateTransformationResource() {
...
void SkiningModel::CreateDirectionalLightResource() {
	// ライトのリソースを作る
...
void SkiningModel::CreateCameraResource() {
...
void SkiningModel::CreatePointLightResource() {
...
void SkiningModel::CreateSpotLightResource() {
